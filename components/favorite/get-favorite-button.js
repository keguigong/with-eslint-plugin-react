...
const thunkRequest = (operation = 'seek', deviceType, deviceId) => {
  return (dispatch, getState) => {
    ...
    let url = `${process.env.SERVER_URL}/favorite?operation=${operation}&deviceType=${deviceType}&deviceId=${deviceId}&username=${user}`

    dispatchIfValid({ type: GET_FAVORITE_BUTTON_STATE_PENDING })
    fetch(url, {
      headers: {
        Token: token
      }
      ...
      .then(res => {
        if (res.code === 200) {
          dispatchIfValid({
            type: GET_FAVORITE_BUTTON_STATE_SUCCESS,
          ...
          dispatchIfValid({type: GET_FAVORITE_BUTTON_STATE_ERROR, error: res.message})
        }
      })
      .catch(error => {
        dispatchIfValid({type: GET_FAVORITE_BUTTON_STATE_ERROR, error: error.message})
      })
  }
}
